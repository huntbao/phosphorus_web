...
class ReqBuilderBody extends React.Component {
    ...
    return (
      <div className="select-lists">
                <span className={rawTypeClasses}>
                    <span className="select-dis-value" onClick={(e) => {
                      this.toggleRawTypeList(e)
                    }}>
                        <span className="dis-name">{bodyType.name}</span>
                        <span className="glyphicon glyphicon-chevron-down"></span>
                    </span>
                    <DropDownMenu menus={this.props.rawTypes} onClickItem={(v) => {
                      this.onSelectRawTypeValue(v)
                    }}/>
                </span>
        <span className={jsonTypeclasses}>
                    <span className="select-dis-value" onClick={(e) => {
                      this.toggleJSONTypeList(e)
                    }}>
                        <span className="dis-name">{jsonType}</span>
                        <span className="glyphicon glyphicon-chevron-down"></span>
                    </span>
                    <DropDownMenu menus={this.props.jsonTypes} onClickItem={(v) => {
                      this.onSelectJSONTypeValue(v)
                    }}/>
                </span>
      </div>
    )
    ...
    if (jsonKVs.length === 0) {
      ...
      kvvt = (
        <KeyValueVT
          ...
          kvs={jsonKVs}
          toggleKV={(rowIndex, kv) => {
            this.toggleBodyRawJSONKV(rowIndex, kv)
          }}
          addKV={(rowIndex, kv) => {
            this.addBodyRawJSONKV(rowIndex, kv)
          }}
          removeKV={(rowIndex) => {
            this.removeBodyRawJSONKV(rowIndex)
          }}
          changeKVKey={(rowIndex, value) => {
            this.changeBodyRawJSONKVKey(rowIndex, value)
          }}
          changeKVValue={(rowIndex, value) => {
            this.changeBodyRawJSONKVValue(rowIndex, value)
          }}
          changeKVValueType={(rowIndex, value) => {
            this.changeBodyRawJSONKVValueType(rowIndex, value)
          }}
          changeKVChildValueType={(rowIndex, value) => {
            this.changeBodyRawJSONKVChildValueType(rowIndex, value)
          }}
        />
      )
    }
  ...
  getFormDataCon() {
    return (
      <KeyValueX
        ...
        toggleKV={(rowIndex, kv) => {
          this.toggleBodyFormDataKV(rowIndex, kv)
        }}
        addKV={() => {
          this.addBodyFormDataKV()
        }}
        removeKV={(rowIndex) => {
          this.removeBodyFormDataKV(rowIndex)
        }}
        changeKVKey={(rowIndex, value) => {
          this.changeBodyFormDataKVKey(rowIndex, value)
        }}
        changeKVValue={(rowIndex, value) => {
          this.changeBodyFormDataKVValue(rowIndex, value)
        }}
        changeKVValueType={(rowIndex, value) => {
          this.changeBodyFormDataKVValueType(rowIndex, value)
        }}
        changeKVFileValue={(rowIndex, value) => {
          this.changeBodyFormDataKVFileValue(rowIndex, value)
        }}
      />
    )
  }
    ...
    if (this.props.builders.bodyXFormKVs.length === 0) {
      ...
      kv = (
        <KeyValue
          ...
          toggleKV={(rowIndex, kv) => {
            this.toggleBodyXFormKV(rowIndex, kv)
          }}
          addKV={() => {
            this.addBodyXFormKV()
          }}
          removeKV={(rowIndex) => {
            this.removeBodyXFormKV(rowIndex)
          }}
          changeKVKey={(rowIndex, value) => {
            this.changeBodyXFormKVKey(rowIndex, value)
          }}
          changeKVValue={(rowIndex, fileInput) => {
            this.changeBodyXFormKVValue(rowIndex, fileInput)
          }}
        />
      )
    }
  ...
  getBinaryCon() {
    return (
      <input type="file" className="binary-file" name="binaryData" onChange={(e) => {
        this.changeBinaryData(e)
      }}/>
    )
  }
