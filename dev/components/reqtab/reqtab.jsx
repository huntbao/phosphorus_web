...
class ReqTab extends React.Component {
  ...
  render() {
    ...
    let tabNodes = this.props.tabs.map((tab, index) => {
      ...
      return (
        <div className={tabClasses} key={index} title={tab.name}>
          ...
          <span className="reqtab-name" onMouseUp={(e) => {
            this.mouseUp(e, index)
          }}>
                        <em>{tab.name || tab.url || 'New tab'}</em>
                    </span>
          <div className={removeBtnClasses} onMouseUp={(e) => {
            this.remove(e, index)
          }} title="Close tab">
    ...
    let addButton = <div className="glyphicon glyphicon-plus-sign reqtab-add" onClick={() => {
      this.add()
    }}></div>
    return (
