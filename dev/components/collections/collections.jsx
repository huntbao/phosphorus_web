...
class Collections extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = Object.assign({
      isXHRPluginInstalled: false
    }, props)
  }

  getReqNode(req, folder, collection, index) {
    return (
      <Req
        req={req}
      ...
      collectionNodes = collections.map((collection, index) => {
        ...
        let folderNodes = collection.folders.map((folder, index) => {
          ...
          let requestNodes = folder.orders.map((reqId, index) => {
            ...
            return this.getReqNode(request, folder, collection, index)
          })
          return (
            <div className="coll-folder" key={index}>
              <div className="coll-folder-wrap"
                   onClick={(e) => {
                     this.toggleFolderSlide(e)
                   }}
                   onMouseLeave={(e) => {
                     this.onMouseLeaveFolder(e)
                   }}
              >
                ...
                <DropDownMenu
                  ...
                  onClickItem={(menuItem, e) => {
                    this.onClickFolderMenuItem(menuItem, collection, folder, e)
                  }}
                />
              </div>
        ...
        if (notInFolderReqs.length) {
          notInFolderReqNodes = notInFolderReqs.map((req, index) => {
            foldersHeight += 30
            return this.getReqNode(req, null, collection, index)
          })
        }
        ...
        return (
          <div className={collClasses} key={index}>
            <div className="coll-wrap"
                 onClick={(e) => {
                   this.toggleCollSlide(e)
                 }}
                 onMouseLeave={(e) => {
                   this.onMouseLeaveColl(e)
                 }}
            >
              ...
              <div className="coll-actions">
                <div className="coll-actions-menus" onClick={(e) => {
                  this.toggleCollActionMenu(e)
                }}>
                  <em className="glyphicon glyphicon-option-horizontal"></em>
                </div>
              </div>
              <DropDownMenu
                ...
                onClickItem={(menuItem, e) => {
                  this.onClickCollectionMenuItem(menuItem, collection, e)
                }}
              />
            </div>
    ...
    return (
      <div className={className}>
        <div className="mod-collection-actions">
                    <span className="icon-wrap" onClick={(e) => {
                      this.clearLocalStorage(e)
                    }}
        ...
        <div className="mod-copyright">
          {
            !this.state.isXHRPluginInstalled &&
            <div className="xhr-plugin-install-tip">
              This tool is depend on XHR Proxy Tool, <a href="https://chrome.google.com/webstore/detail/fbakmpanchidgmjopcmcddoihgjkfcjn" target="xhr-proxy-tool">Click to install</a>.
            </div>
          }
  ...
  componentDidMount() {
    console.log('Check if XHR proxy tool is installed....')
    setTimeout(() => {
      const sendId = Date.now()
      document.addEventListener('check-xhrpt-ext-res', e => {
        if (!e.detail || !e.detail.reqData || e.detail.reqData.sendId !== sendId) {
          return
        }
        console.log('XHR proxy tool is installed!')
        this.setState({
          isXHRPluginInstalled: true
        })
      }, false)
      const event = new CustomEvent('check-xhrpt-ext', {
        detail: {
          sendId: sendId
        }
      })
