  ...
  render() {
    ...
    let getNodes = (kvs, parentKV, parentValueType, parentChildValueType) => {
      return kvs.map((kv, index) => {
        ...
        return (
          <div className={rowClasses} key={index} onMouseOut={(e) => {
            this.mouseOutRow(e)
          }} onMouseOver={(e) => {
            this.mouseOverRow(e)
          }}>
            <div className="kv-row-wrap">
              <div className={okSignClasses} onClick={() => {
                this.toggle(kv.index, kv)
              }}></div>
              {inputs}
              {titleTip}
              <div className="glyphicon glyphicon-remove" onClick={() => {
                this.remove(kv.index)
              }}></div>
            </div>
  ...
  getInputs(kv, rowIndex, keyInputProps, valueInputProps) {
    ...
    let getChildNodes = () => {
      if (kv.valueType === 'array') {
        ...
        return (
          <select value={kv.childValueType} onChange={(e) => {
            this.changeKVChildValueType(rowIndex, e)
          }} disabled={!kv.childTypeChangeable}>
    ...
    return (
      <div className={classes} onFocus={(e) => {
        ...
        <input {...keyInputProps} />
        <select value={valueType} onChange={(e) => {
          this.changeKVValueType(rowIndex, e)
        }} disabled={!kv.typeChangeable}>
